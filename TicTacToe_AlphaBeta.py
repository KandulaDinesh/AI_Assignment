...
def minimax(state, player, alpha=-INFINITY, beta=+INFINITY):
    ...
    if player == AI_PLAYER:
        ...
        for move in get_possible_moves(state):
            ...
            if check_winner(new_state, player):
                return 1, move  # take the winning move
            score, _ = minimax(new_state, HUMAN_PLAYER, alpha, beta)
            if score > max_score:
                max_score = score
                best_move = move
        ...
        for move in get_possible_moves(state):
            new_state = make_move(state, move, player)
            if check_winner(new_state, player):
                return -1, move  # block the winning move
            score, _ = minimax(new_state, AI_PLAYER, alpha, beta)
            if score < min_score:
                min_score = score
                best_move = move
            beta = min(beta, score)
